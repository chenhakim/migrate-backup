...
//            }
            ...
            $out = '$this->batchInsert(\'{{%' . $this->getTableName($table) . '}}\',[';
            $this->upStr->addStr($out);
            $str_columns = '';
            foreach ($tableSchema->columns as $column) {
                $str_columns .= "'" . $column->name . "'," ;
            }
            $str_columns = rtrim($str_columns, ',');
            // 先期输出
            $this->upStr->addStr($str_columns);
            $this->upStr->addStr( '], [');
            ...
            foreach ($data as $row) {
                ...
                foreach ($tableSchema->columns as $column) {
                    ...
                    if (is_null($row[ $column->name ])) {
                        ...
                        /* 序列化的内容被addslashes就不能反序列化了 */
                        $str_data .= "'" . $row[ $column->name ] . "',";
                    } else {
                        $str_data .= "'" . addslashes($row[ $column->name ]) . "',";
                    }
                }
                $str_data = rtrim($str_data, ',') . '],';
//                $str_data .= PHP_EOL; // 换行能不能实现呢
                $this->upStr->addStr($str_data);
            }
